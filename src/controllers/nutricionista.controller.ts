...
export const atualizarDadosNutricionista = async (
  req: Request,
  res: Response
) => {
  const { id } = req.params;
...
export const alterarSenhaNutricionista = async (
  req: Request,
  res: Response
) => {
  const { id } = req.params;
  ...
  try {
    const nutricionistaAtualizado = await service.alterarSenha(
      parseInt(id),
      dto
    );
    return res.status(200).json(nutricionistaAtualizado);
...
export const alterarTemaDoNutricionista = async (
  req: Request,
  res: Response
) => {
  try {
...
export const buscarTotalDeConsultasDoMes = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const dados = await service.totalConsultasDoMes(parseInt(id));

    return res.status(200).json({
      total: dados.totalMesAtual,
      totalMesAnterior: dados.totalMesAnterior,
      crescimento: dados.crescimento,
    });
  } catch (error: any) {
    logger.error("Erro ao buscar total de consultas do mês:", error);
    return res.status(500).json({ error: "Erro interno do servidor" });
  }
};

export const buscarTotalDeConsultasDosUltimos12Meses = async (req: Request, res: Response) => {
  try {
    const { id } = req.params

    const dados = await service.ultimos12MesesConsultas(parseInt(id));

    return res.status(200).json(dados);
  } catch (error: any) {
    logger.error("Erro ao buscar total de consultas dos últimos 12 meses:", error);
    return res.status(500).json({ error: "Erro interno do servidor" });
  }
};
